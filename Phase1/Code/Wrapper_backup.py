...
# Iterate over each folder (image set)
 # Ensure it's a directory
    ...
    # Apply Shi-Tomasi Corner Detection
        corners = cv2.goodFeaturesToTrack(image_corner_preprocessing, maxCorners=800, qualityLevel=0.005, minDistance=10)
    ...
    match_pairs_ransac, H = match_features_with_ransac(match_pairs,image_data, match_dir_RANSAC, ransac_thresh=5.0, max_iters=1000)
    for i in range(len(H)):
        print("Homography Matrix: ", np.round(H[i], 3))
