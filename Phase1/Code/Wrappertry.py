...
def StitchImages(BaseImage, SecImage,HomographyMatrix):
...
def stitch_images(image1, image2, homography_matrix):
    """
    Stitches two images together using a given homography matrix.

    Parameters:
    - image1: The first image (base image).
    - image2: The second image to warp and stitch onto the first.
    - homography_matrix: The homography matrix (3x3 numpy array) that transforms image2 to align with image1.
...
# Read images from the folder and store them in a list images
    ...
    if set_name == 'CustomSet3':

        center_index = 3  # Python indexing starts at 0, so images[3] is the 4th image
        left_indices = list(range(center_index - 1, -1, -1))  # [2, 1, 0] for left-side images
        right_indices = list(range(center_index + 1, len(images)))  # [4, 5, 6, 7] for right-side images

        stitching_order = []  # Final sequence of images for stitching

        # Interleave left and right indices
        for l, r in zip(left_indices, right_indices):
            stitching_order.extend([l, r])

        # Add any remaining indices (if images are odd-numbered)
        stitching_order.extend(left_indices[len(right_indices):])
        stitching_order.extend(right_indices[len(left_indices):])

        # Initialize stitching with the 4th image
        current_merge = images[center_index]

        for i, img_idx in enumerate(stitching_order):
            current_image = images[img_idx]  # Get next image in stitching order
            print(f'Stitching image {img_idx + 1} ({i + 1}/{len(stitching_order)})')
            ...
            # Perform ANMS
            num_best = 800
            marked_image_corners, selected_corners_image = ANMS(image_corners, num_best, current_image)
            marked_merge_corners, selected_corners_merge = ANMS(merge_corners, num_best, current_merge)
            ...
            # Extract features
            image_FD, FD_image_current = features(current_image, selected_corners_image)
            merge_FD, FD_image_merge = features(current_merge, selected_corners_merge)

            # Match features
            matched_pairs = match_features(current_merge, current_image, image_FD, merge_FD, selected_corners_image, selected_corners_merge, match_dir, i)
            match_pairs_ransac, H = match_features_with_ransac(matched_pairs, current_merge, current_image, image_FD, merge_FD, selected_corners_image, selected_corners_merge, match_dir_RANSAC, i, ransac_thresh=3.0, max_iters=2000)

            # Warp and blend images
            # warped_image, offset_x, offset_y = warp(current_merge, H[0])
            # result_blended = blending2images(current_image, warped_image, offset_x, offset_y)

            result_blended = stitch_images(current_image, current_merge, H[0])
            cv2.imwrite(f"stitched_image_{i+1}.jpg", result_blended)

            # Update current_merge for next iteration
            current_merge = np.uint8(result_blended)

            # # Save output images
            # image_final = Image.fromarray(result_blended.astype('uint8'), 'RGB')
            # image_final.save(f"stitched_image_{i+1}.jpg")
        ...
        for i in range(len(images)-1):
            ...
            #current_image=ProjectOntoCylinder(images[i+1])
