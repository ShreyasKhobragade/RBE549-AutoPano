...
# Read images from the folder and store them in a list images
    ...
    for i in range(len(images)-1):
        ...
        print('loop :', i)
        matched_pairs = match_features(current_image,current_merge,merge_FD,image_FD,selected_corners_merge_corners,selected_corners_image_corners,match_dir,i)
        match_pairs_ransac, H = match_features_with_ransac(matched_pairs,current_image,current_merge,merge_FD,image_FD,selected_corners_merge_corners,selected_corners_image_corners,match_dir_RANSAC, i,ransac_thresh=5.0, max_iters=1000)
        ...
        warped_image_source1, source1_offset_x, source1_offset_y = warp(current_image, H[0])
        image1 = Image.fromarray(warped_image_source1.astype('uint8'), 'RGB')
        image1.save("warped_image.jpg")
        result_blended = blending2images( warped_image_source1,current_merge, source1_offset_x, source1_offset_y)
        ...
        current_merge = np.uint8(result_blended)#!/usr/bin/evn python
...
# Read images from the folder and store them in a list images
    ...
    for i in range(len(images)-1):
        ...
        match_pairs_ransac, H = match_features_with_ransac(matched_pairs,current_merge,current_image,image_FD,merge_FD,selected_corners_image_corners,selected_corners_merge_corners,match_dir_RANSAC, i,ransac_thresh=5.0, max_iters=1000)
        # match_pairs_ransac, H = match_features_with_ransac(matched_pairs,current_merge,current_image,image_FD,merge_FD,selected_corners_image_corners,selected_corners_merge_corners,match_dir_RANSAC, i,ransac_thresh=15.0, max_iters=1000)
# 
        ...
        if i>0:
            h = int(np.ceil(current_merge.shape[1]*0.9))
            w = int(np.ceil(current_merge.shape[0]*0.9))
            result_blended = cv2.resize(result_blended, (h,w))
        print(current_image.shape[1], current_image.shape[0])
